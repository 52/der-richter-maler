...
}
{
  ...
  window.__loadedJsLibraries =
    window.__loadedJsLibraries || (window.__loadedJsLibraries = new Object());
  window.__loadedCssLibraries =
    window.__loadedCssLibraries || (window.__loadedCssLibraries = new Object());
  if (
    ...
    cm4all.SmartAccess = {
      ...
      detectMobile: function () {
        cm4all.SmartAccess.smartAccessButton = document.querySelector(
          ".cm-smart-access-button"
        );
        cm4all.SmartAccess.smartAccessMenu = document.querySelector(
          ".cm-smart-access-menu"
        );
        cm4all.SmartAccess.smartAccessProtector = document.querySelector(
        ...
        if (
          ...
          (document.location.hash.indexOf("mobile") >= 0 ||
            ...
            (window.Common &&
              ...
              Common.MobileBrowserSwitch.isMobileAgent(navigator.userAgent)))
        ) {
          ...
          if (cm4all.SmartAccess.smartAccessMenu) {
            cm4all.SmartAccess.smartAccessButton.onclick =
              cm4all.SmartAccess.toggleMenu;
            cm4all.SmartAccess.smartAccessMenu.addEventListener(
              "transitionend",
              cm4all.SmartAccess.onTransitionEnd
            );
            cm4all.SmartAccess.smartAccessProtector.onclick =
              cm4all.SmartAccess.closeMenu;
      ...
      onHashChange: function (ev, initializing) {
        ...
        if (document.location.hash.indexOf("smartAccess") >= 0) {
          cm4all.SmartAccess.smartAccessButton
            .querySelector(".fa")
            .classList.remove("fa-th");
          /* trigger css transition */
          cm4all.SmartAccess.smartAccessButton.offsetHeight;
          cm4all.SmartAccess.smartAccessButton
            .querySelector(".fa")
            .classList.add("fa-close");
          cm4all.SmartAccess.smartAccessButton.querySelector(
            ".scaler"
          ).style.transform = "";
      ...
      onScroll: function (ev) {
        if (!cm4all.SmartAccess.menuOpen) {
          if (
            !isNaN(cm4all.SmartAccess.scrollY) &&
            window.scrollY === cm4all.SmartAccess.scrollY
          ) {
            return;
          }
  ...
  if (
    ...
    (function () {
      ...
      function replaceWithAll(oldNode, newNodes, callback) {
        ...
        allSaved = [].map.call(all.childNodes, function (n) {
          return n;
        });
        ...
        allSaved.forEach(function (e) {
          ...
          var links = [].map.call(
            e.querySelectorAll("a[href],form[action]"),
            function (a) {
              return a;
            }
          );
      ...
      function displayThirdpartyContent() {
        if (this.checked) {
          var inputList = document.querySelectorAll(
            ".cm-wp-content-switcher__checkbox"
          );
      ...
      function showWidgetPlaceholder(a) {
        ...
        if (!document.querySelector('[data-ph-wiid="' + wiid + '"]')) {
          var template = document.querySelector(
            ".cm4all-cookie-policy-placeholder-template"
          );
          if (template) {
            ...
            placeholder
              .querySelector("input")
              .addEventListener("change", displayThirdpartyContent);
      ...
      function showWidgetLoadingPlaceholder(a) {
        ...
        if (!document.querySelector('[data-ph-wiid="' + wiid + '"]')) {
          ...
          placeholder.innerHTML =
            '<img src="' +
            window.beng.env.common_prefix +
            '/.cm4all/e/static/img/loading_128.gif"></img>';
          placeholder.dataset.phWiid = wiid;
          a.insertAdjacentElement("afterend", placeholder);
        }
      }
      ...
      if ("IntersectionObserver" in window) {
        lazyWidgetObserver = new IntersectionObserver(function (
          entries,
          observer
        ) {
          entries.forEach(function (entry) {
            if (entry.isIntersecting) {
              ...
              var lazyWidgetAnchor = lazyWidgetContainer.querySelector(
                "a.cm4all-cookie-consent"
              );
      ...
      function _loadWidget(widgetAnchor, followElement, evt, callback) {
        ...
        xhr.onreadystatechange = function () {
          ...
          if (xhr.status === 200) {
            var html = DOM_PARSER.parseFromString(
              "<x-cm4all-root>" + xhr.responseText + "</x-cm4all-root>",
              "text/html"
        ...
        setTimeout(function () {
          if (followElement.nodeName === "A") {
            ...
            xhr.open(
              (followElement.method || "POST").toUpperCase(),
              followElement.action
            );
            ...
            if (followElement.enctype === "application/x-www-form-urlencoded") {
              for (entry of formData.entries()) {
                toSend +=
                  "&" +
                  encodeURIComponent(entry[0]).replace(/%20/g, "+") +
                  "=" +
      ...
      function loadWidgets() {
        ...
        var widgetAnchorList = document.querySelectorAll(
          "a.cm4all-cookie-consent"
        );
        ...
        for (var i = 0; i < widgetAnchorList.length; i++) {
          ...
          var allowRendering =
            ...
            ((cookieSettings.thirdparty === true ||
              a.dataset.thirdparty !== "true") &&
              (cookieSettings.statistics === true ||
                a.dataset.statistics !== "true"));
          if (!allowRendering) {
        ...
        if (
          ...
          cookieSettings.tracking === false
        ) {
          var scriptList = document.querySelectorAll(
            'script[type="application/x-cm4all-cookie-consent"]'
          );
          for (var i = 0; i < scriptList.length; i++) {
            var script = scriptList[i];
            var html = DOM_PARSER.parseFromString(
              "<x-cm4all-root>" + script.dataset.code + "</x-cm4all-root>",
              "text/html"
            ).querySelector("x-cm4all-root").childNodes;
        ...
        var untrustedWidgetAnchorList = document.querySelectorAll(
          "a.cm4all-untrusted-widget"
        );
    ...
    */
        ...
        [].forEach.call(
          document.querySelectorAll("a.cm4all-untrusted-widget"),
          function (a) {
            var iframeId =
              a.id.replace(/^anchor/, "iframe") + "_" + window.iframe_suffix;
